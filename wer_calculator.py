...
def transcribe_with_openai(audio_path: str) -> tuple[str, float]:
    import time
...
if __name__ == "__main__":
    print("=" * 65)
    print("STT Benchmark — Clean Run")
    print("=" * 65)

    nr_configs = {
        "raw": NRProcessingConfig(
            state="raw",
            tool="none",
            tool_version="n/a",
            params={},
    ...
    results = []
    versions = {"openai_api": "gpt-4o-mini-transcribe"}
    ...
    condition = AudioCondition(
        ...
        channel="mono",
        codec="mp3",
        notes="Clean Common Voice clips",
    )

    nr_state = "raw"
    nr_config = nr_configs[nr_state]

    csv_path = "data/test_transcripts.csv"
    audio_dir = "data/audio/clean"
    ...
    with open(csv_path, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            hypothesis_text, latency = transcribe_with_openai(audio_file)
            ...
            results.append(
                build_result(
                    ...
                    nr_config=nr_config,
                    engine="openai_api",
                    engine_version=versions["openai_api"],
                    engine_config={"model": "gpt-4o-mini-transcribe"},
                    reference_text=reference_text,
                    hypothesis_text=hypothesis_text,
    ...
    run = BenchmarkRun(
        run_id=run_id,
        description="Clean real-data run",
        created_utc=datetime.utcnow().isoformat() + "Z",
        engines_tested=["openai_api"],
        conditions_tested=["clean"],
        nr_states_tested=[nr_state],
        normalization_config={"remove_fillers": REMOVE_FILLERS},
